...
export class ForgotPasswordAttemptsDao {
  ...
  async getForgotPasswordAttemptsByUser(usrID: number) {
    try {
      return await this.forgotPasswordAttemptsRepository.find({
        where: {
          user: { usr_id: usrID },
        }
      });
  ...
  async deleteForgotPasswordAttemptsFisicaById(fpaID: number): Promise<void> {
    try {
      await this.forgotPasswordAttemptsRepository.delete(fpaID);
    } catch (error) {
      throw new BadRequestException({
        statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
        message: [`${error.message}`],
